...
bool IGameOfLifeInstance::getCellByIndex(vector<int>
                                         &board,
                                         int index
) {
...
int IGameOfLifeInstance::countAliveNeighbours(vector<int>
                                              &gameState,
                                              int index
) {
    int count = 0;
    count +=
            getCellByIndex(gameState, index
                                      - width - 1) ? 1 : 0;
    count +=
            getCellByIndex(gameState, index
                                      - width) ? 1 : 0;
...
bool IGameOfLifeInstance::updateCell(vector<int>
                                     &gameState,
                                     int index
) {
...
SerialGameOfLife::SerialGameOfLife(int width, int height) : IGameOfLifeInstance(width, height) {
}

void SerialGameOfLife::executeStep(vector<int>
                                   &oldState,
                                   vector<int> &newState
) {
    for (
...
ParallelGameOfLife::ParallelGameOfLife(int width, int height, int threadCount) : IGameOfLifeInstance(width, height) {
    this->threadCount = threadCount;
    threadStride = getBoardSize() / threadCount;
}

void ParallelGameOfLife::executeStep(vector<int>
                                     &oldState,
                                     vector<int> &newState
) {
    tbb::parallel_for(0, threadCount, [&](
            size_t threadIndex
    ) {
        for (
                int index = threadIndex * threadStride;
                index < (threadIndex + 1) *
                        threadStride;
                index++) {
            newState[index] =
